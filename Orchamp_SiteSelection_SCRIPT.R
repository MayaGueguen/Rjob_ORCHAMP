...
params = expand.grid(prob1 = seq(0,1,0.1)
                     , prob2 = seq(0,1,0.1)
                     , prob3 = seq(0,1,0.1))
params = params[which(params$prob3 >= params$prob2),]
# params = params[1:2,]
year.end = 2035
...
HOP = foreach(prob1 = params$prob1
              , prob2 = params$prob2
              , prob3 = params$prob3, .combine = 'rbind') %dopar%
{
  cat("\n", prob1)
  ...
  len = foreach(rep = 1:3, .combine = "rbind") %do%
  {
    ...
    # return(length(which(table(RES$SEL$SITE) < 5)))
